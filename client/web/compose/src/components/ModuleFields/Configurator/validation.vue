  ...
  >
    ...
    <div>
      ...
      <div class="mt-4">
        ...
        >
          <i>
            {{ $t('constraints.totalFieldConstraintCount', { total: fieldConstraint.total, interpolation: { escapeValue: false } }) }}
          </i>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    checkForFieldConstraint () {
      for (let x = 0; x < this.module.config.recordDeDup.rules.length; x++) {
        const { constraints } = this.module.config.recordDeDup.rules[x]

        for (let i = 0; i < constraints.length; i++) {
          const value = constraints[i]

          if (value.attribute === this.field.name) {
