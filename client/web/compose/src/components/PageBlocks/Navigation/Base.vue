<template>
  <wrap
    ...
    v-on="$listeners"
  >
    <div class="h-100 w-100 card overflow-hidden">
      <b-nav
      ...
      >
        <b-nav-item
          ...
          :key="`navItem-${index}`"
          :to="toggleToLink(navItem)"
          :href="toggleHrefLink(navItem)"
        ...
        >
          <template v-if="navItem.type === 'dropdown' || navItem.options.item.displaySubPages">
            <b-button
              :id="`dropdown-popover-${index}`"
          ...
          <template v-else-if="!navItem.options.item.displaySubPages">
...
export default {
  ...
  methods: {
    getSubPages (selfID) {
      return this.pages.filter(value => value.selfID === selfID) || []
    },
    ...
    toggleToLink (navItem) {
      if (navItem.type === 'dropdown' || navItem.options.item.displaySubPages) {
        return
      }

      return navItem.options.item.pageID
    },

    toggleHrefLink (navItem) {
      if (navItem.type === 'dropdown' || navItem.options.item.displaySubPages) {
        return
      }
