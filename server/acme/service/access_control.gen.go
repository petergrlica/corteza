...
func (svc accessControl) Resources() []rbac.Resource {
	return []rbac.Resource{
		rbac.NewResource(types.CuserRbacResource(0)),
...
func (svc accessControl) List() (out []map[string]string) {
	def := []map[string]string{
		{
			"type": types.CuserResourceType,
			"any":  types.CuserRbacResource(0),
			"op":   "cuser.create",
		},
		{
...
// CanCreateCuserOnCuser checks if current user can create custom user
//
// This function is auto-generated
func (svc accessControl) CanCreateCuserOnCuser(ctx context.Context, r *types.Cuser) bool {
	return svc.can(ctx, "cuser.create", r)
}

// CanGrant checks if current user can manage acme permissions
//
// This function is auto-generated
...
func rbacResourceValidator(r string, oo ...string) error {
	...
	case types.CuserResourceType:
		return rbacCuserResourceValidator(r, oo...)
...
func (svc accessControl) resourceLoader(ctx context.Context, resource string) (rbac.Resource, error) {
	...
	case types.CuserResourceType:
		if hasWildcard {
			return rbac.NewResource(types.CuserRbacResource(0)), nil
		}

		return loadCuser(ctx, svc.store, ids[0])
	case types.ComponentResourceType:
...
// rbacCuserResourceValidator checks validity of RBAC resource and operations
//
// # Can be called without operations to check for validity of resource string only
//
// This function is auto-generated
func rbacCuserResourceValidator(r string, oo ...string) error {
	if !strings.HasPrefix(r, types.CuserResourceType) {
	...
	for _, o := range oo {
		if !defOps[o] {
			return fmt.Errorf("invalid operation '%s' for cuser resource", o)
		}
	}

	const sep = "/"
	var (
		pp  = strings.Split(strings.Trim(r[len(types.CuserResourceType):], sep), sep)
		prc = []string{
			"ID",
		}
	)
	...
	for i := 0; i < len(pp); i++ {
		if pp[i] != "*" {
			if i > 0 && pp[i-1] == "*" {
				return fmt.Errorf("invalid path wildcard level (%d) for cuser resource", i)
			}
...
func rbacComponentResourceValidator(r string, oo ...string) error {
	...
	for _, o := range oo {
		if !defOps[o] {
			return fmt.Errorf("invalid operation '%s' for acme component resource", o)
		}
	}
