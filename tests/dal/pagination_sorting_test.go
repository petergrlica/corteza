...
func TestSortingAndPagination(t *testing.T) {
	...
	{
		t.Log("setting up DAL")
		ds, err = dal.New(zap.NewNop(), true)
		require.NoError(t, err)
		require.NoError(t, ds.ReplaceConnection(
			ctx,
			dal.MakeConnection(1, testApp.Store.ToDalConn(),
	...
	var (
		...
		truncAndCreate = func(t *testing.T, rr ...*types.Record) (*require.Assertions, types.RecordSet) {
			req := require.New(t)
			_, err := testApp.Store.(*rdbms.Store).DB.ExecContext(ctx, "DELETE FROM "+recordsTable)
			req.NoError(err)
		...
		str = func(set types.RecordSet, fields ...string) string {
			var out string
			for r := range set {
				if r > 0 {
					out += ";"
	...
	t.Run("search", func(t *testing.T) {
		t.Run("by record attributes", func(t *testing.T) {
			...
			set, _, err = dalutils.ComposeRecordsList(ctx, ds, mod, types.RecordFilter{})
			req.NoError(err)
			req.Len(set, valid) // we've deleted one

			// search for ALL
			set, _, err = dalutils.ComposeRecordsList(ctx, ds, mod, types.RecordFilter{Deleted: filter.StateInclusive})
			req.NoError(err)
			req.Len(set, count) // we've deleted one

			// search for deleted only
			set, _, err = dalutils.ComposeRecordsList(ctx, ds, mod, types.RecordFilter{Deleted: filter.StateExclusive})
			req.NoError(err)
		...
		t.Run("by values", func(t *testing.T) {
			...
			f.Query = `str1 = 'v1'`
			set, _, err = dalutils.ComposeRecordsList(ctx, ds, mod, f)
			req.NoError(err)
			req.Len(set, 1)
		...
		t.Run("with like op", func(t *testing.T) {
			var (
				...
				_, _, err = dalutils.ComposeRecordsList(ctx, ds, mod, f)
			)
			req.NoError(err)
		})
		...
		for _, tc := range tcc {
			t.Run("crawling: "+tc.sort, func(t *testing.T) {
				...
				for p := 0; p < 3; p++ {
					...
					req.Equal(tc.rval[p], str(set, "str1", "str3"))
				...
				for _, tc := range tcc {
					...
					t.Run("crawling: "+k.kind+"; "+tc.sort, func(t *testing.T) {
						...
						for p := 0; p < 4; p++ {
							...
							ff := p * 2
							req.Equal(expToString(k.ptrn, tc.exp[ff:ff+int(f.Limit)]), str(set, strings.ToLower(k.kind)+"1", strings.ToLower(k.kind)+"2", strings.ToLower(k.kind)+"3"))
			...
			for _, tc := range tcc {
				t.Run("crawling: "+tc.sort, func(t *testing.T) {
					...
					for p := 0; p < 2; p++ {
						...
						req.NoError(err)
						req.True(tc.sort == f.Sort.String() || strings.HasPrefix(f.Sort.String(), tc.sort+","))
						req.Equal(tc.rval[p], str(set, "bool1", "bool3"))
	...
	t.Run("sort by system field, paged", func(t *testing.T) {
		...
		set, f, err = dalutils.ComposeRecordsList(ctx, ds, mod, f)
		req.NoError(err)
		req.Equal("v3;v4;v5", str(set, "str1"))
		req.Nil(f.NextPage)
	...
	t.Run("paged", func(t *testing.T) {
		...
		req.NotNil(f.NextPage)
		req.Nil(f.PrevPage)
		req.Equal("v1,w3.1;v2,w3.2;v3,w3.3", str(set, "str1", "str3"))

		f.PageCursor = f.NextPage
		set, f, err = dalutils.ComposeRecordsList(ctx, ds, mod, f)
		req.NoError(err)
		req.Equal("v4,<NIL>;v5,<NIL>;v6,<NIL>", str(set, "str1", "str3"))
		...
		set, f, err = dalutils.ComposeRecordsList(ctx, ds, mod, f)
		req.NoError(err)
		req.Equal("w3.3,v3", str(set, "str3", "str1"))
		...
		for p := 1; p <= 5; p++ {
			...
			set, f, err = dalutils.ComposeRecordsList(ctx, ds, mod, f)
			req.NoError(err)
			req.Equal(fmt.Sprintf("v%d", validOrderOfStr1[p]), str(set, "str1"))
			//fmt.Printf("%-30s\t%v\t%30s\t%30s\n", str(set, "str1", "str3"), f.Sort, f.NextPage, f.PrevPage)
			f.PageCursor = f.NextPage
		}
		...
		for p := 1; p <= 6; p++ {
			...
			set, f, err = dalutils.ComposeRecordsList(ctx, ds, mod, f)
			req.NoError(err)
			//fmt.Printf("%-30s\t%v\t%30s\t%30s\n", str(set, "str3", "str1"), f.Sort, f.NextPage, f.PrevPage)
			req.Equal(fmt.Sprintf("v%d", validOrderOfStr1[p]), str(set, "str1"))
			f.PageCursor = f.NextPage
		}
	...
	t.Run("page navigation", func(t *testing.T) {
		...
		req.NoError(err)
		req.NotNil(set)
		req.Equal("v1", str(set, "str1"))
	})

	t.Run("report", func(t *testing.T) {
		t.Skip("needs adaptations for DAL")
	...
	t.Run("partial value update", func(t *testing.T) {
		...
		//set, _, err = dalutils.ComposeRecordsList(ctx, ds, mod, types.RecordFilter{})
		//req.NoError(err)
		//req.Equal("1st,1;2nd,2;3rd,3", str(set, "str1", "num1"))
		//
		...
		//set, _, err = dalutils.ComposeRecordsList(ctx, ds, mod, types.RecordFilter{})
		//req.NoError(err)
		//req.Equal("1st,1;2nd,22;3rd,3", str(set, "str1", "num1"))
	})
	...
	t.Run("date-time filtering", func(t *testing.T) {
		...
		for _, c := range cases {
			t.Run(c.query, func(t *testing.T) {
				req = require.New(t)
				set, _, err = dalutils.ComposeRecordsList(ctx, ds, mod, types.RecordFilter{Query: c.query})
				req.NoError(err)
				req.Equal(c.result, str(set, "num1"))
			})
		}
	})
}
